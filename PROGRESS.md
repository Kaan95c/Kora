...
> **Dernière mise à jour : 2026-06-11 — étape 24 : RECHERCHE TOPBAR FONCTIONNELLE (`GET /api/search` + modal `SearchCommand` ⌘K, debounce 300 ms, résultats groupés Contacts/Projets/Documents) + LIENS DASHBOARD câblés (View All → `/projects`, View Full Calendar → `/scheduler`, CTA tâches urgentes) + MENU QUICK ACTION (Sidebar → drawers via `?new=1` + hook `useNewDrawerParam` + `<Suspense>` dans AppShell). Avant : étape 23 = FORGOT PASSWORD. Avant : étape 22 = GOOGLE OAUTH. Avant : étape 21 = PAGES LÉGALES + bandeau cookies. Avant : étape 20 = GATING / QUOTAS PAR PLAN. Avant : étapes 16→18 = MISE EN PRODUCTION (Vercel) + Stripe LIVE + abonnements SaaS + responsive mobile. Étapes 1 à 15 = 9 pages + Supabase + Stripe + PDF + Resend + portail client. Prochaine : onboarding (wizard), relances auto, `/projects/[id]`.**
