...
- **⚠️ TODO infra (hors code, côté user)** : (1) pointer le domaine **`kora-app.fr`** sur le projet Vercel (le définir comme **Production Domain**) ; (2) mettre à jour **Supabase → Auth → URL Configuration** : *Site URL* = `https://kora-app.fr` + *Redirect URLs* (`https://kora-app.fr/auth/callback`) sinon OAuth/reset password cassent ; (3) si Google OAuth, vérifier les *Authorized origins/redirect* côté Google Cloud. Si `app.kora-app.fr` doit rester valide en transition, **ajouter** `https://app.kora-app.fr` à l'allowlist CORS (sinon ses appels API seront bloqués).
